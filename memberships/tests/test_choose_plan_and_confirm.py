...
@pytest.mark.django_db
def test_confirm_creates_pending_subscription(
    client, user, player_senior, product_senior_current, monkeypatch
):
    ...
    url = (
        reverse("memberships:confirm", args=[player_senior.id, 0])
        + f"?product={product_senior_current.id}"
    )
...
@pytest.mark.django_db
def test_duplicate_membership_blocked_by_unique_constraint(
    client, user, player_senior, product_senior_current, monkeypatch
):
    client.login(username="alice", password="pw")
    monkeypatch.setattr("memberships.views.localdate", lambda: date(2024, 12, 1))
