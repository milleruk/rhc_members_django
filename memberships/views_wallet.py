...
def apple_wallet_pkpass(request, public_id):
    ...
    if (
        getattr(player, "created_by_id", None)
        and player.created_by_id != request.user.id
    ...
    if not getattr(settings, "WALLET_APPLE_ENABLED", False):
        ...
        resp["Content-Disposition"] = (
            f'attachment; filename="RHC-{player.membership_number}-TEST.pkpass"'
        )
        return resp
